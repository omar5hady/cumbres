...
class MedioPublicitarioController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($buscar==''){
            $medios_publicitarios = Medio_publicitario::orderBy('nombre','desc')->paginate(5);
        }
        else{
            $medios_publicitarios = Medio_publicitario::where($criterio, 'like', '%'. $buscar . '%')->orderBy('nombre','desc')->paginate(5);
        }
