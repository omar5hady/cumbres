...
class InstitucionFinanciamientoController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($buscar==''){
            $instituciones_financiamiento = InstitucionFinanciamiento::orderBy('nombre','desc')->paginate(9);
        }
        else{
            $instituciones_financiamiento = InstitucionFinanciamiento::where($criterio, 'like', '%'. $buscar . '%')->orderBy('nombre','desc')->paginate(9);
        }
