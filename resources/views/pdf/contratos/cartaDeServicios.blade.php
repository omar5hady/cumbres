...
@if($datos[0]->modelo == "Terreno")
  <img style="margin-top: -20px;" src="files/etapas/plantillasCartaServicios/{{ $datos[0]->plantilla_carta_servicios2 }}" width="794" height="1020">
@else
...
<div style="margin-top: 200px; position: absolute; width: 100%; left:60px; right:60px;">
    ...
    @if($datos[0]->emp_constructora == "CONCRETANIA")
      <p align="left">Bienvenido a la Familia Concretania Sr (a) <u>{{$datos[0]->nombre}} {{$datos[0]->apellidos}}</u></p>
    @else
    ...
    @else
      <p align="justify">&nbsp; &nbsp; Queremos informarte por la presente que para mantener la Belleza y Plusvalia del Fraccionamiento 
            se manejara una cuota de mantenimiento la cual tendrá como objetivo el solventar los gastos comunes tales como: 
      </p>
    @endif
...
@endfor
  ...
  @if($datos[0]->modelo == "Terreno")
    <p align="justify">&nbsp; &nbsp; La aportacion será de <strong>${{$datos[0]->costoMantenimientoLetra2}}</strong> de forma mensual, como costo base;
        cabe destacar que al momento de construir en el Lote la cuota se incrementa a <strong>${{$datos[0]->costoMantenimientoLetra}}</strong>, 
        cuota que estarán pagando actualmente las Viviendas del Fraccionamiento. Las cuotas pueden actualizarse conforme a los requerimientos de los costos 
