            ...
            <div class="col-md-10">
                <div class="input-group">
                    ...
                    <select class="form-control" v-if="search.criterio=='lotes.fraccionamiento_id'"
                        ...
                        <option v-for="fraccionamientos in $root.$data.proyectos" :key="fraccionamientos.id"
                            :value="fraccionamientos.id" v-text="fraccionamientos.nombre">
                        </option>
                    </select>
                    <select class="form-control" v-else-if="search.criterio=='expedientes.gestor_id'" v-model="search.buscar">
                        <option value="">Gestor</option>
                        <option v-for="gestor in $root.$data.arrayGestores" :key="gestor.id"
                            :value="gestor.id" v-text="gestor.nombre_gestor">
                        </option>
                    </select>
                    <input v-else class="form-control" placeholder="Texto a buscar"
                        type="text" v-model="search.buscar"
                        @keyup.enter="$emit('listarData', search)">
                </div>
                <div class="input-group" v-if="search.criterio=='lotes.fraccionamiento_id'">
                    <select class="form-control col-md-6"  v-model="search.b_etapa">
                        ...
                        <option v-for="etapas in $root.$data.etapas" :key="etapas.id"
                            :value="etapas.id" v-text="etapas.num_etapa">
                        </option>
                    </select>
                </div>
                <div class="input-group"  v-if="search.criterio=='lotes.fraccionamiento_id'">
                ...
                <tr v-for="item in arrayData" :key="item.id">
                    ...
                    <td class="td2">
                        ...
                        <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 39px, 0px);">
                            <a v-if="item.saldo < 0" class="dropdown-item" target="_blank"
                                :href="`/devoluciones/printSolicDevolucion?id=${item.folio}`">
                                Solic. Devolución
                            </a>
                    ...
                    <td class="td2" v-text="item.nombre_cliente"></td>
                    ...
                    <td class="td2" >
                        <a lass="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
                    ...
                    <td v-if="item.aviso_prev=='0000-01-01'" class="td2">
                        No aplica
                    </td>
                    <td v-if="item.aviso_prev!='0000-01-01' && !item.aviso_prev_venc"
                        @dblclick="$emit('abrirModal', { opcion: 'fecha_recibido', data: item })" class="td2"
                        v-text="'Fecha solicitud: '+ this.moment(item.aviso_prev).locale('es').format('DD/MMM/YYYY')">
                    </td>
                    ...
                    <td class="td2" v-if="band==1">
                        <input type="date" @keyup.esc="band=0" v-on:change="actualizarVigencia(item.folio,$event.target.value)" :id="item.folio" :value="item.fecha_vigencia" class="form-control Fields" >
                    </td>
                    ...
                    <td class="td2" @click="$emit('abrirModal', { opcion: 'firma_esc_act', data: item })" v-if="item.fecha_firma_esc"
                    ...
                    <td class="td2" v-else>
                        ...
                        @click="$emit('abrirModal', { opcion: 'firma_esc', data: item })">Generar</button>
                    </td>
                    ...
                    <td class="td2">
                        <button title="Ver todas las observaciones" type="button" class="btn btn-info pull-right"
                            @click="$emit('verObs', item.folio)">Ver Observaciones</button>
                    </td>
                </tr>
            </template>
        </TableComponent>
