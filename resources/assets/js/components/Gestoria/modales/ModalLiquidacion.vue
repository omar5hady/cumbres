<template>
    <ModalComponent :titulo="titulo" @closeModal="$emit('closeModal')">
        <template v-slot:body>
            ...
            <RowModal
                label1="Tipo de crédito"
                clsRow1="col-md-3"
                id1="credito"
                label2="Inst. Financiamiento"
                clsRow2="col-md-3"
                id2="int_fin"
            >
                <input
            ...
            <RowModal
                label1="Fecha firma de contrato"
                id1="fecha_firma_contrato"
                clsRow1="col-md-3"
            >
                <input
                    type="date"
                    disabled
                    v-model="data.fecha_firma_contrato"
                    class="form-control"
                />
            </RowModal>
            ...
            <RowModal
                label1="Valor de escrituración"
                id1="valor_escrituras"
                clsRow1="col-md-3"
                label2=""
                clsRow2="col-md-3"
            >
                ...
                <template v-slot:input2>
                    <h6
                        v-text="'$' + $root.formatNumber(data.valor_escrituras)"
                    ></h6>
                </template>
            </RowModal>

            <RowModal
                v-if="data.intereses_terreno > 0"
                label1="Intereses"
                id1="intereses_terreno"
                clsRow1="col-md-3"
            >
                <h6
                    v-text="'$' + $root.formatNumber(data.intereses_terreno)"
                ></h6>
            </RowModal>

            <template v-if="data.arrayGastos.length">
                <RowModal
                    v-for="gasto in data.arrayGastos"
                    :key="gasto.id"
                    :label1="gasto.concepto"
                    clsRow1="col-md-3"
                >
            ...
            <RowModal
                ...
                id1="pagado"
                clsRow1="col-md-3"
                :label2="data.avaluo ? 'Resultado avaluo' : ''"
                id2="avaluo"
                :clsRow2="data.avaluo ? 'col-md-2' : ''"
            >
                <h6
                    v-text="
                        '$' + $root.formatNumber((data.pagado = data.pagos))
                    "
                ></h6>
                <template v-slot:input2>
                    <h6
                        v-if="data.avaluo"
                        v-text="'$' + $root.formatNumber(data.avaluo)"
                    ></h6>
                </template>
            </RowModal>
            ...
            <RowModal
                v-if="data.descuento > 0"
                label1="Observación del descuento"
                clsRow1="col-md-8"
                id1="obs_descuento"
            >
                <textarea
                    ...
                    cols="30"
                    class="form-control"
                    v-model="data.obs_descuento"
                    placeholder="Observaciones"
                ></textarea>
            </RowModal>

            <RowModal
                label1="Crédito autorizado"
                clsRow1="col-md-3"
                id1="monto_credito"
                label2=""
                clsRow2="col-md-3"
            >
                <input
                    v-model="data.monto_credito"
                    @keyup.enter="updateMontoCredito()"
                    pattern="\d*"
                    @keypress="$root.isNumber($event)"
                    class="form-control"
                />
            ...
            <RowModal
                v-if="
                    data.credito == 'Alia2' ||
                        data.credito == 'Respalda2' ||
                        data.credito == 'INFONAVIT-FOVISSSTE' ||
                        data.credito == 'Fovissste para todos'
                "
                label1="Fovisste"
                clsRow1="col-md-2"
                id1="fovisste"
            ...
            <RowModal
                v-if="
                    data.credito.toLowerCase().includes('cofinavit') ||
                        data.credito == 'Infonavit Mas Banco'
                "
                label1="Infonavit"
                clsRow1="col-md-2"
                id1="infonavit"
            ...
            >
                <h6>
                    <strong>
                        ${{
                            $root.formatNumber(
                                (data.total_liquidar = totalLiquidar)
                            )
                        }}
                    </strong>
                </h6>
            ...
            <RowModal label1="Notas" id1="notas_liquidacion" clsRow1="col-md-8">
                <textarea
                    ...
                    cols="30"
                    class="form-control"
                    v-model="data.notas_liquidacionnotas_liquidacion"
            ...
            >
                <table class="table table-bordered table-striped table-sm">
                    ...
                    <tbody>
                        <tr
                            v-for="(pago, index) in data.arrayPagares"
                            :key="index"
                        >
                            <td v-text="'Pago no. ' + parseInt(index + 1)"></td>
                            <td
                                v-text="
                                    this.moment(pago.fecha_pago)
                                        .locale('es')
                                        .format('DD/MMM/YYYY')
                                "
                            ></td>
            ...
            <div v-show="errorLiquidacion" class="form-group row div-error">
                <div class="text-center text-error">
                    <div
                        v-for="error in errorMostrarMsjLiquidacion"
                        :key="error"
                        v-text="error"
                    ></div>
                </div>
        ...
        <template v-slot:buttons-footer>
            <Button icon="icon-check" @click="generarLiquidacion()"
                >Generar</Button
            >
        </template>
    </ModalComponent>
...
export default {
    ...
    computed: {
        totalLiquidar: function() {
            ...
            let me = this;
            neto_credito =
                parseFloat(me.data.valor_venta) +
                parseFloat(me.data.intereses_terreno) -
                parseFloat(me.data.descuento) +
                parseFloat(me.data.totalGastos) -
                parseFloat(me.data.monto_credito) -
                parseFloat(me.data.infonavit) -
                parseFloat(me.data.fovissste) -
                parseFloat(me.data.pagos);
            return neto_credito;
        },
        netoCredito: function() {
            var total = 0;
            total =
                parseFloat(this.data.infonavit) +
                parseFloat(this.data.fovissste) +
    ...
    methods: {
        updateMontoCredito() {
            ...
            axios
                .put("/update/montocredito/liquidacion", {
                    id: this.data.id,
        ...
        validarLiquidacion() {
            ...
            if (this.data.valor_escrituras == "")
                this.errorMostrarMsjLiquidacion.push(
                    "Ingresar el valor a escriturar."
                );

            if (this.data.descuento === "")
                this.errorMostrarMsjLiquidacion.push("Ingresar descuento.");

            if (this.errorMostrarMsjLiquidacion.length)
        ...
        generarLiquidacion() {
            ...
            //Con axios se llama el metodo update de LoteController
            axios
                .put("/expediente/generarLiquidacion", {
                    folio:              this.data.id,
        ...
        mostrarPagares(precio) {
            ...
            axios
                ...
                .then(function(response) {
                    ...
                    me.data.totalRestante =
                        respuesta.calculos[0].total_restante;
                    me.data.pagos = respuesta.depositos.pagado;
                    me.data.precio_venta =
                        precio + respuesta.calculos[0].interesGen;
                })
                .catch(function(error) {
                    console.log(error);
                });
            ...
            axios
                ...
                .then(function(response) {
                    ...
                    if (respuesta.gastos.length > 0) {
                        ...
                        me.data.totalIntOrd =
                            respuesta.totalIntereses[0].sumIntereses;
                    }
                    console.log(me.data)
                })
                .catch(function(error) {
