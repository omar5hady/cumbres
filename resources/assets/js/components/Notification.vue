        ...
        <div class="dropdown-menu dropdown-menu-right">
            ...
            <div v-if="notifications.length">
                <li v-for="item in listar" :key="item.id">
                    ...
                    <a class="dropdown-item dropdown-item2" href="#" @click="cambiar(item.data.datos.notificacion)">
                        <div class="message">
                            ...
                            <div>
                                <small class="text-muted">{{
                                    item.data.datos.notificacion.usuario
                                }}</small>
                                <small
                                    class="text-muted float-right mt-1"
                                    v-text="
                                        this.moment(
                                            item.data.datos.notificacion.fecha
                                                .date,
                                            'YYYY-MM-DD hh:mm:ss'
                                        )
                                            .locale('es')
                            ...
                            <div class="font-weight-bold">
                                ...
                                ></span>
                                {{ item.data.datos.notificacion.titulo }}
                            </div>
            ...
            <div v-else>
                <a class="dropdown-item text-center"
                    ><span>No tiene notificaciones</span></a
                >
            </div>
            <div>
                <a
                    class="dropdown-item text-center"
                    @click="$root.$data.menu = 101"
                    ><i class="fa fa-user"></i> Ver mas...</a
                >
            </div>
        </div>
    </li>
...
export default {
        ...
        listar: function() {
            ...
            if (this.notifications == "") {
                ...
                if (this.notifications.length < 5)
                    ...
                    return (this.arrayNotificaciones = this.notifications.slice(
                        0,
                        4
                    ));
    ...
    methods:{
        cambiar(notificacion){
            if(notificacion.menu){
                this.$root.$data.menu = notificacion.menu;
            }
        }
    }
};
