                    ...
                    <template v-else>
                        ...
                        <div v-if="listado == 1" class="card-body">
                            ...
                            <TableComponent :cabecera="[
                                '# Ref','Proyecto','Etapa','Manzana','Lote','Modelo','Fecha solicitud','Fecha Programada para visita','Status'
                            ]">
                                <template v-slot:tbody>
                                    <tr v-for="contratos in arrayContratos" :key="contratos.solicitudID" @dblclick="listado=2,listarDetalles(contratos.solicitudID)" title="Ver Detalle">
                                        ...
                                        <td class="td2">
                                            <span class="badge" :class="[contratos.status == 2 ? 'badge-success' : 'badge-warning']">
                                                {{ (contratos.status == 0) ? 'Pendiente'
                                                    : (contratos.status == 1) ? 'En Proceso'
                                                    : 'Concluido'
                                                }}
                                            </span>
                                        </td>
                        ...
                        <div v-if="listado==2" class="card-body">
                            ...
                            <TableComponent :cabecera="[
                                '# Ref','General','Subconcepto','Detalle','Descripción','Garantia','Costo','Fecha concluido','Resultado'
                            ]">
                                <template v-slot:tbody>
                                    <tr v-for="detalles in arrayDetalles" :key="detalles.solicitud_id">
                                        ...
                                        <td>
                                            <span class="badge" :class="[detalles.garantia == 1 ? 'badge-success' : 'badge-danger']">
                                                {{ (detalles.garantia == 1) ? 'Si' : 'No' }}
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            <span class="badge" :class="[detalles.revisado == 0 ? 'badge-warning'
                                                : detalles.revisado == 1 ? 'badge-danger'
                                                : 'badge-success'
                                            ]">
                                                {{ (detalles.revisado == 0) ? 'Pendiente'
                                                    : (detalles.revisado == 1) ? `Rechazado ${detalles.resultado}` : 'Aprobado'
                                                }}
                                            </span>
                                        </td>
                                    </tr>
