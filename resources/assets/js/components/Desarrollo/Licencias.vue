            ...
            <div class="container-fluid">
                ...
                <div class="card scroll-box">
                    <div class="card-header">
                        ...
                        <!--   Boton   -->
                        <a v-if="rolId != '5'" class="btn btn-success" v-bind:href="'/licencias/resume_excel'" title="Descargar resumen">
                            <i class="icon-pencil"></i>&nbsp; Resumen
                        </a>
                        <!---->
                        <template v-if="allLic.length > 0 && rolId != '5'">
                            <Button :btnClass="'btn-info'" @click="abrirModal('asignarMasa')"
                                :icon="'fa fa-pencil-square-o'" title="Actualizar seleccion">
                            </Button>
                            <Button :btnClass="'btn-default'" @click="abrirModal('subirArchivo')"
                                :icon="'icon-cloud-upload'" title="Subir archivos por seleccion">
                            </Button>
                        </template>
                        <Button v-if="criterio=='lotes.fraccionamiento_id' && buscar != ''"
                            :btnClass="'btn-scarlet'" @click="abrirModal('subirArchivoProyecto')"
                            :icon="'icon-cloud-upload'" title="Subir archivos al proyecto">
                            Carga por proyecto
                        </Button>
                        <Button v-if="criterio=='lotes.fraccionamiento_id' && buscar != ''"
                            :btnClass="'btn-scarlet'" @click="abrirModal('verArchivosProyecto')"
                            :icon="'icon-eye'" title="Ver archivos del proyecto">
                            Ver por proyecto
                        </Button>
                            ...
                            <template v-slot:tbody>
                                <template v-if="tipoAccion == 1">
                                    <tr v-for="p in archivos" :key="p.id">
                                        <td class="td2">
                                            <button
                                                title="Eliminar archivo" type="button" @click="deleteFile(p.id)" class="btn btn-danger btn-sm">
                                ...
                                <template v-else>
                                    <tr v-for="p in archivos" :key="p.file_id">
                                        <td class="td2">
                                            <Button title="Eliminar Archivo por proyecto" :icon="'icon-trash'" @click="deleteDocByProyecto(p.file_id)"
                                                :btnClass="'btn-danger'" :size="'btn-sm'">
                                            </Button>
                                        </td>
                                        <td class="td2" v-text="p.tipo"></td>
                                        <td class="td2">
                                            <a :href="p.public_url" target="_blank" class="btn btn-success">Ver Archivo</a>
                                        </td>
                                    </tr>
                                </template>
                            </template>
...
import LoadingComponent from '../Componentes/LoadingComponent.vue'
    ...
    export default {
        ...
        methods : {
            ...
            deleteDocByProyecto(id){
            let me = this;
            swal({
                title: '¿Desea eliminar este archivo?',
                text: "Esta documento se elimiinara en todos los lotes",
                type: 'warning',
                ...
                if (result.value) {
                    me.loading = true
                    axios.delete('/docs/deleteDocByProyecto', {
                            params: {'file_id': id}
                    }).then(function (response){
                        me.loading = false
                        me.listarLicencias(me.pagination.current_page,me.buscar,me.b_manzana,me.b_lote,me.b_modelo,me.b_arquitecto,me.criterio,me.buscar2);
                        me.archivos = me.archivos.filter( e => e.file_id !== id)
                        //Se muestra mensaje Success
                        const toast = Swal.mixin({
                            toast: true,
                            position: 'top-end',
            ...
            cambiarPagina(page){
                ...
                me.listarLicencias(page,me.buscar,me.b_manzana,me.b_lote,me.b_modelo,me.b_arquitecto,me.criterio,me.buscar2);
            },
            selectEtapas(buscar){
                let me = this;
                me.b_etapa="";
            ...
            getDocsByProyecto(buscar){
                let me = this;
                me.archivos = [];
                var url = '/docs/getDocsByProyecto?proyecto=' + buscar;
                axios.get(url).then(function (response) {
                    const respuesta = response.data;
                    me.archivos = respuesta;
                })
                .catch(function (error) {
                    console.log(error);
                });
            ...
            abrirModal(accion,data =[]){
                switch(accion){
                    ...
                    case 'verArchivosProyecto':{
                        this.modal = 6;
                        this.tituloModal = 'Archivos cargados en proyecto';
                        this.getDocsByProyecto(this.buscar);
                        this.tipoAccion = 2;
                        break;
                    }
