    ...
    :cabecera="[
        ...
        <template v-slot:tbody>
            <tr v-for="avisoObra in arrayAvisoObra" :key="avisoObra.id" title="Ver detalle">
                <td class="td2">
                    <ButtonComponent icon="fa fa-upload" v-if="(usuario == 'lupita.vidales' || usuario == 'uriel.al' || usuario == 'shady') && avisoObra.acuse_cierre == null && avisoObra.acuse_contratista != null"
                        title="Acuse de cierre contratante"
                        @click="$emit('abrirModal',{accion:'subirAcuse',data: avisoObra})"
                    >
                        Cargar Acuse de cierre
                    </ButtonComponent>
                    <ButtonComponent icon="fa fa-upload" v-if="(rolId == 13 || usuario == 'shady') && avisoObra.acuse_contratista == null"
                        title="Acuse de cierre contratista"
                        @click="$emit('abrirModal',{accion:'subirAcuseContratista',data: avisoObra})"
                    >
                        Cargar Acuse de cierre
                    </ButtonComponent>
                </td>
                <td>
                    ...
                    <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 39px, 0px);">
                        ...
                        <template v-if="avisoObra.total_importe != (avisoObra.total_original + avisoObra.total_extra)">
                            <a class="dropdown-item" v-if="rolId!=9 && rolId != 11 && rolId!=13"
                                @click="$emit('abrirModal',{accion:'subirAdendum',data: avisoObra})">
                                <i class="icon-cloud-upload"></i>
                                Subir Adendum
                            </a>
                            <a class="dropdown-item" v-if="avisoObra.adendum2 == null"
